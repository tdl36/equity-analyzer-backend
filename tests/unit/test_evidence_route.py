...
class EvidenceRouteTests(unittest.TestCase):
    def setUp(self):
        ...
        class Cursor:
            ...
            def fetchall(self): return [{'id':'old','state':{'thesis':['Preserved thesis']}}]
        @contextmanager
        ...
        exec(compile(ast.Module(body=[function],type_ignores=[]),'app_v3.py','exec'),{'app':app,'get_db':get_db,'re':re,'jsonify':jsonify})
    ...
    def test_get_reads_two_versions_and_marks_legacy_unchecked(self):
        ...
        self.assertNotIn('UPDATE',self.calls[0][0])
