...
var esc = value => String(value ?? 'N/A').replace(/[&<>"']/g, ch => ({
  '&': '&amp;',
  '<': '&lt;',
  '>': '&gt;',
  '"': '&quot;',
...
var safeUrl = u => /^https?:\/\//i.test(String(u || '')) ? String(u) : '#';
var listHTML = (items, renderer = x => esc(x)) => (items || []).map(x => `<li>${renderer(x)}</li>`).join('');
var TEMPLATE_NOTES = {
  twopager: {
    notebook: 'Readable notebook edition: same investment content across two pages with larger type.',
    institutional: 'Professional two-page investment brief with larger tables and charts.'
  },
  onepager: {
...
async function health() {
  try {
    ...
    var d = await r.json();
    $('status').textContent = d.openai_enabled ? `Ready · AI + web research enabled · model ${d.model}` : 'Ready · OpenAI key not set. DE calibration + market-data mode available.';
  } catch (e) {
    $('status').textContent = 'Cannot reach the local backend. Restart the app and refresh.';
  }
}
async function analyze() {
  var ticker = $('ticker').value.trim().toUpperCase();
  if (!ticker) return;
  ...
  try {
    ...
    var cached = d.meta?.cached ? ' · cache' : '';
    var violations = d.meta?.onepager_violations?.length || 0;
...
function updateTopMeta() {
  ...
  var m = CURRENT.master;
  if (OUTPUT === 'report') $('topMeta').textContent = `${m.company} · ${$('reportTemplate').selectedOptions[0].textContent}`;else if (OUTPUT === 'onepager') $('topMeta').textContent = `${m.company} · ${$('onepagerTemplate').selectedOptions[0].textContent}`;else $('topMeta').textContent = `${m.company} · ${$('twopagerTemplate').selectedOptions[0].textContent}`;
}
...
function logoHTML(d, cls = 'company-logo') {
  var website = CURRENT?.master?.at_glance?.website || d.identity?.website || '';
  var domain = websiteDomain(website);
  if (!domain) return `<div class="ticker-fallback">${esc(d.ticker)}</div>`;
  var src = `https://www.google.com/s2/favicons?domain_url=${encodeURIComponent('https://' + domain)}&sz=128`;
...
function reportHeader(m, page, descriptor = '') {
  var g = m.at_glance || {};
  var pairs = [['Share Price', g.share_price], ['Market Cap', g.market_cap], ['Exchange', g.exchange], ['Sector', g.sector], ['Industry', g.industry], ['As of', g.data_as_of]];
...
function reportFooter(m, p) {
  return `<div class="report-footer"><span>${esc(m.ticker)} · Equity Research</span><span>Page ${p} / 3</span></div>`;
}
/* Share for a segment.
 *
...
function hasProfitMix(segments) {
  ...
  var vals = segs.map(x => rawShare(x, 'profit'));
...
function rawShare(x, basis) {
  var numeric = basis === 'profit' ? Number(x?.profit_mix_numeric) : Number(x?.mix_numeric ?? x?.value);
  if (Number.isFinite(numeric) && numeric > 0) return numeric;
  var label = basis === 'profit' ? x?.profit_mix : x?.mix;
...
function reportPieSVG(items, cls = 'report-pie-svg') {
  var vals = (items || []).map(segmentShare),
    total = vals.reduce((a, b) => a + b, 0) || 1;
  var cols = ['#87ad58', '#91b9dc', '#e3bf55', '#ad94c9'];
  var a = 0;
  var paths = vals.map((v, i) => {
    var a1 = a + v / total * 360,
      mid = (a + a1) / 2;
    var p = `<path d="${pieArc(150, 150, 104, a, a1)}" fill="${cols[i % cols.length]}" stroke="#555" stroke-width="1.05"/>`;
    // A wedge too thin to hold its label gets none: the label cannot fit inside
    // the slice and lands on its neighbour's instead.
    var label = '';
    if (v / total >= 0.06) {
      var pp = piePoint(150, 150, 67, mid);
...
function finNums(value) {
  return (String(value || '').replace(/\([^)]*\)/g, ' ') // "(2027E)", "(±25bps)"
...
function multipleNums(value) {
  var s = String(value || '');
  var marked = [...s.matchAll(/(\d+(?:\.\d+)?)\s*[x\u00d7]/gi)].map(m => Number(m[1]));
...
function pickSpread(sorted, n) {
  ...
  var span = sorted[sorted.length - 1] - sorted[0] || sorted[0] || 1;
  var tol = Math.max(span * 0.06, 0.4);
  var u = [];
  sorted.forEach(v => {
    if (!u.some(x => Math.abs(x - v) < tol)) u.push(v);
  });
  if (u.length >= n) {
    var _out = [];
    for (var i = 0; i < n; i++) _out.push(u[Math.round(i * (u.length - 1) / (n - 1))]);
    var ded = [];
    _out.forEach(v => {
      if (!ded.includes(v)) ded.push(v);
    });
    while (ded.length < n) {
      var gap = (ded[ded.length - 1] - ded[0]) / Math.max(1, ded.length - 1) || ded[0] * .1;
      ded.push(Math.round((ded[ded.length - 1] + gap) * 100) / 100);
    }
    return ded.slice(0, n);
  }
  if (!u.length) return [];
  var gaps = [];
  for (var _i = 1; _i < u.length; _i++) gaps.push(u[_i] - u[_i - 1]);
  var step = gaps.length ? gaps.reduce((a, b) => a + b, 0) / gaps.length : Math.max(u[0] * .12, .5);
  var out = u.slice();
...
function earningsPattern(history) {
  var vals = (history?.points || []).map(p => Number(p.value)).filter(Number.isFinite);
  ...
  for (var i = 2; i < vals.length; i++) {
    var a = Math.sign(vals[i - 1] - vals[i - 2]);
    var b = Math.sign(vals[i] - vals[i - 1]);
...
function earningsLabels(history) {
  var cyclical = earningsPattern(history) === 'cyclical';
  return {
    cyclical,
    chartTitle: cyclical ? 'Earnings Are Cyclical' : 'Earnings Trajectory',
    chartHead: cyclical ? 'Earnings Cycle' : 'Earnings Trajectory',
    matrixTitle: cyclical ? 'Mid-Cycle EPS \u00d7 P/E Sensitivity' : 'Forward EPS \u00d7 P/E Sensitivity',
...
function reportSensitivityHTML(m, f) {
  var scenarios = m.valuation_scenarios || [];
  var current = parseMoneyNumber(m.at_glance?.share_price);
  ...
  var eps = [];
  scenarios.forEach(x => {
    finNums(x.earnings).forEach(n => {
      if (n > 0 && n < 1000) eps.push(n);
    });
  });
  if (!eps.length) eps = finNums(f.eps).filter(n => n > 0 && n < 1000);
  if (!eps.length) eps = [current ? current / 15 : 10];
  var rows = pickSpread(eps.slice().sort((a, b) => a - b), 4).map(v => Math.round(v * 100) / 100);
  ...
  var scenarioMults = [];
  scenarios.forEach(x => {
    multipleNums(x.multiple).forEach(n => {
      if (n > 0 && n < 100) scenarioMults.push(n);
    });
  });
  var contextMults = [];
  multipleNums(f.historical_pe).forEach(n => {
    if (n > 0 && n < 100) contextMults.push(n);
  });
  multipleNums(f.forward_pe).forEach(n => {
    if (n > 0 && n < 100) contextMults.push(n);
  });
  var mults = scenarioMults.slice();
  contextMults.forEach(n => {
    if (mults.length < 4 && !mults.some(x => Math.abs(x - n) < Math.max(0.75, n * 0.08))) mults.push(n);
  });
  ...
  /* Reject multiples that are not part of the working range. Historical-P/E
     ...
     otherwise. */
  var anchorCands = finNums(f.forward_pe).filter(n => n > 0 && n < 100);
  var sortedM = mults.slice().sort((a, b) => a - b);
  var anchor = anchorCands.length ? anchorCands[0] : sortedM[Math.floor(sortedM.length / 2)];
  var banded = sortedM.filter(n => n >= anchor * 0.5 && n <= anchor * 2.2);
  var cols = pickSpread(banded.length >= 2 ? banded : sortedM, 4).map(v => Math.round(v * 10) / 10);
  var fmtEps = e => Number.isInteger(e) ? String(e) : e.toFixed(2).replace(/0$/, '');
  var cell = (e, mult) => {
    var v = Math.round(e * mult);
    var c = 'neutral';
    if (current) {
      var r = v / current;
      c = r < .9 ? 'down' : r > 1.1 ? 'up' : 'near';
    }
    return `<td class="${c}"><b>$${v.toLocaleString()}</b></td>`;
  };
...
function reportDecisionLensHTML(t, includeVariant = true) {
  var constructive = (t.what_must_be_true || []).slice(0, 3);
  var breaks = (t.falsification || []).slice(0, 3);
...
function tpSignpostTableHTML(items) {
  var rows = (items || []).slice(0, 6).map(x => `<tr><td><b>${esc(x.signpost)}</b></td><td>${nbRich(x.current)}</td><td>${nbRich(x.target)}</td><td>${esc(x.why)}</td></tr>`).join('');
...
function reportScenarioMatrixHTML(items) {
  var rows = (items || []).slice(0, 3).map(x => `<tr class="${String(x.case || '').toLowerCase()}"><th>${esc(x.case)}</th><td><b>${esc(x.earnings)}</b></td><td><b>${esc(x.multiple)}</b></td><td><b>${esc(x.implied_value)}</b></td><td>${esc(x.logic)}</td></tr>`).join('');
...
function reportScenarioSideHTML(items) {
  var rows = (items || []).slice(0, 3).map(x => `<div class="report-side-scenario ${String(x.case || '').toLowerCase()}"><b>${esc(x.case)}</b><span>${esc(x.earnings)}</span><span>${esc(x.multiple)}</span><strong>${esc(x.implied_value)}</strong><small>${esc(x.logic)}</small></div>`).join('');
  return `<div class="report-side-scenarios"><h3>Scenario Framework</h3>${rows}</div>`;
}
function reportDecisionMatrixHTML(items) {
  var rows = (items || []).slice(0, 3).map(x => `<tr class="${String(x.case || '').toLowerCase()}"><th>${esc(x.case)}</th><td>${esc(x.earnings)}</td><td>${esc(x.multiple)}</td><td>${esc(x.implied_value)}</td><td>${esc(x.logic)}</td></tr>`).join('');
...
function renderReport() {
  if (!CURRENT) return;
  var m = CURRENT.master,
    t = m.investment_thesis || {},
    o = m.company_overview || {},
    f = m.financial_snapshot || {};
  var pools = (m.business_model || []).slice(0, 4).map(x => `<div class="report-pool"><b>${esc(x.name)}</b><span>${esc(x.description)}</span></div>`).join('');
  var segments = (o.segments || []).slice(0, 4).map(x => `<div class="report-segment"><div><b>${esc(x.name)}</b><strong>${esc(x.mix)}</strong></div><p>${esc(x.description)}</p></div>`).join('');
  var opps = (m.opportunities || []).slice(0, 5).map(x => `<div class="report-opp"><b>${esc(x.title)}</b><p>${esc(x.detail)}</p></div>`).join('');
  var primaryMetrics = [['Revenue', f.revenue, f.revenue_context], ['Operating Margin', f.operating_margin, f.margin_context], ['EPS', f.eps, f.eps_context], ['Free Cash Flow', f.free_cash_flow, f.fcf_context], ['Leverage', f.leverage, 'Balance sheet'], ['Returns', f.returns, 'Capital efficiency']];
  var metrics = primaryMetrics.map(x => `<div class="report-metric"><div class="value">${esc(x[1])}</div><div class="label">${esc(x[0])}</div><div class="context">${esc(x[2] || '')}</div></div>`).join('');
  var targets = (f.management_targets || []).slice(0, 4).map(x => `<div class="report-target"><span>${esc(x.label)}</span><b>${esc(x.value)}</b><small>${esc(x.context || '')}</small></div>`).join('');
  var signs = (m.signposts || []).slice(0, 6).map(x => `<tr><td><b>${esc(x.signpost)}</b></td><td>${esc(x.current)}</td><td>${esc(x.target)}</td><td>${esc(x.why_it_matters)}</td></tr>`).join('');
  var cats = (m.catalysts || []).slice(0, 4).map(x => `<div class="report-catalyst"><b>${esc(x.timing)}</b><strong>${esc(x.event)}</strong><span>${esc(x.why_it_matters)}</span></div>`).join('');
  var threats = (m.thesis_threats || []).slice(0, 4).map(x => `<div class="report-threat"><b>${esc(x.threat)}</b><p>${esc(x.watch_for)}</p></div>`).join('');
  var decisionMatrix = reportDecisionMatrixHTML(m.valuation_scenarios || []);
  var reportChart = earningsChartSVG(m.earnings_history || CURRENT.onepager?.earnings_history || {}, 'report-earnings-chart');
  setSegmentBasis(o.segments);
  var companyPie = reportPieSVG((o.segments || []).slice(0, 4));
  var other = (o.other_profit_pools || []).join(' · ');
...
function iconHTML(name) {
  return `<span class="line-icon">${ICONS[name] || ICONS.product}</span>`;
}
function donutHTML(items, cls = '') {
  var vals = items.map(x => segmentShare(x));
  var total = vals.reduce((a, b) => a + b, 0) || 1;
  var cur = 0;
  var colors = ['var(--c1)', '#83aee0', '#e9c75c', '#b89bd0'];
  var stops = [];
  vals.forEach((v, i) => {
    var s = cur / total * 100;
    cur += v;
    var e = cur / total * 100;
    stops.push(`${colors[i % colors.length]} ${s}% ${e}%`);
  });
...
function flowHTML(items, cls = '') {
  return `<div class="causal-flow ${cls}">${items.map((x, i) => `${i ? '<span class="flow-arrow">→</span>' : ''}<div class="flow-node"><b>${esc(x.label)}</b><small>${esc(x.detail)}</small></div>`).join('')}</div>`;
}
...
function valueLabelsSVG(pts, x, y, W, out) {
  ...
  return pts.map((d, i) => {
    ...
    var yy = Math.min(y(Number(d.value)) + 15, 126);
    placed.push({
      x0,
      x1: x0 + w,
      y: yy
    ...
    return `<text x="${cx.toFixed(1)}" y="${yy.toFixed(1)}" text-anchor="${anchor}" ` + `class="vlab${d.kind === 'estimate' ? ' est' : ''}">${esc(text)}</text>`;
  }).join('');
}
function annotationsSVG(pts, x, y, W, occupied) {
  var CHAR_W = 4.6; // ~px per character at the 9px annotation size
  var LINE = 10;
  var BOTTOM = 132; // keep clear of the x-axis labels
  var MAX_CH = 34; // a label wider than this cannot fit any placement
  var placed = Array.isArray(occupied) ? occupied.slice() : [];
  return pts.map((d, i) => ({
    d,
    i
  })).filter(o => o.d.annotation).map(o => {
    ...
    var text = String(o.d.annotation);
    if (text.length > MAX_CH) return '';
    var w = text.length * CHAR_W;
    var cx = x(o.i),
      anchor = 'middle';
    if (cx - w / 2 < 2) {
      anchor = 'start';
    ...
    /* Try above the point first, then below. Stacking upward alone ran into
       ...
       destroys both. */
    var base = y(Number(o.d.value));
    var free = yy => yy >= 9 && yy <= BOTTOM && !placed.some(pl => !(x1 < pl.x0 - 2 || x0 > pl.x1 + 2) && Math.abs(yy - pl.y) < LINE);
    var chosen = null;
    for (var k = 1; k <= 5 && chosen === null; k++) {
      var up = base - 10 - (k - 1) * LINE;
      if (free(up)) chosen = up;
    }
    for (var _k = 1; _k <= 4 && chosen === null; _k++) {
      var down = base + 12 + (_k - 1) * LINE;
    ...
    if (Array.isArray(occupied)) occupied.push({
      x0,
      x1,
      y: chosen
    });
...
function earningsChartSVG(history, className = '') {
  var pts = (history?.points || []).filter(x => Number.isFinite(Number(x.value))).slice(-12);
  ...
  var vals = pts.map(x => Number(x.value));
  var min = Math.min(...vals, 0),
    max = Math.max(...vals);
  if (max === min) max = min + 1;
  var x = i => p.l + i * (W - p.l - p.r) / (pts.length - 1);
  var y = v => p.t + (max - v) * (H - p.t - p.b) / (max - min);
  var actualIdx = pts.map((x, i) => x.kind === 'estimate' ? i : null).filter(x => x !== null)[0] ?? pts.length;
  var path = (arr, offset = 0) => arr.map((d, i) => `${i ? 'L' : 'M'}${x(i + offset).toFixed(1)},${y(Number(d.value)).toFixed(1)}`).join(' ');
  var actual = pts.slice(0, Math.max(2, actualIdx));
  var estimateStart = Math.max(0, actualIdx - 1);
  var estimate = pts.slice(estimateStart);
  var ticks = [min, (min + max) / 2, max];
...
function sectionTitle(n, title, extra = '') {
  return `<h2><span class="sec-num">${n}</span><span>${esc(title)}</span>${extra ? `<small>${esc(extra)}</small>` : ''}</h2>`;
}
...
function companyMotifs(d, m) {
  var t = String(d.ticker || '').toUpperCase(),
    sector = String(m?.at_glance?.sector || '').toLowerCase(),
    industry = String(m?.at_glance?.industry || '').toLowerCase();
  var kinds = ['gear'];
  if (t === 'DE' || industry.includes('agric') || industry.includes('machinery')) kinds = ['tractor', 'excavator'];else if (industry.includes('semiconductor')) kinds = ['chip', 'cloud'];else if (sector.includes('health') || industry.includes('biotech') || industry.includes('drug')) kinds = ['vial', 'gear'];else if (sector.includes('financial') || industry.includes('bank')) kinds = ['bank', 'cloud'];else if (sector.includes('energy') || industry.includes('oil')) kinds = ['oil', 'gear'];else if (industry.includes('aerospace') || industry.includes('airline')) kinds = ['plane', 'gear'];else if (sector.includes('technology') || industry.includes('software')) kinds = ['cloud', 'chip'];
...
function poolIconName(name = '') {
  ...
  if (s.includes('finance')) return 'policy';
  if (s.includes('service') || s.includes('after')) return 'replacement';
  if (s.includes('software') || s.includes('technology') || s.includes('data')) return 'data';
  return 'product';
}
...
function nbFrame(x, y, w, h, cls = '') {
  var j = (x + y + w + h) % 7 - 3,
    ...
    x2 = x + w,
    y2 = y + h;
  var d = `M ${x + r} ${y + .6 * j} Q ${x + w * .35} ${y - .5 * j} ${x2 - r} ${y + .35 * j} Q ${x2 + 1} ${y} ${x2 + .3 * j} ${y + r} L ${x2 - .25 * j} ${y2 - r} Q ${x2} ${y2 + 1} ${x2 - r} ${y2 - .3 * j} Q ${x + w * .55} ${y2 + .5 * j} ${x + r} ${y2 - .25 * j} Q ${x - 1} ${y2} ${x + .2 * j} ${y2 - r} L ${x - .2 * j} ${y + r} Q ${x} ${y} ${x + r} ${y + .6 * j} Z`;
...
var LEGAL_SUFFIX_RE = /,?\s+(incorporated|corporation|company limited|limited|holdings? (plc|inc|ltd)|plc|inc\.?|corp\.?|ltd\.?|s\.a\.?|n\.v\.?|ag|sa|nv)$/i;
function displayCompany(d) {
  var full = String(d.company || '').trim();
  var ticker = String(d.ticker || '');
  ...
  for (var i = 0; i < 3; i++) {
    var next = short.replace(LEGAL_SUFFIX_RE, '').trim();
...
function nbTitleSize(d) {
  ...
  var n = displayCompany(d).length + String(d.ticker || '').length + 3;
  var BAND_PX = 596,
    PER_CHAR_EM = 0.62;
  var fitted = Math.floor(BAND_PX / (n * PER_CHAR_EM));
...
function pieArc(cx, cy, r, a0, a1) {
  var p0 = piePoint(cx, cy, r, a0),
    p1 = piePoint(cx, cy, r, a1),
    large = a1 - a0 > 180 ? 1 : 0;
...
function segmentBasis(d) {
  ...
  var segs = (d.segments || []).filter(x => segmentShare(x) > 0);
  var total = segs.reduce((a, x) => a + segmentShare(x), 0);
  if (!segs.length) return '';
  var what = SEGMENT_BASIS === 'profit' ? 'segment operating profit' : 'revenue';
  return Math.abs(total - 100) > 6 ? `(share of ${what}, indicative)` : `(share of ${what})`;
}
function pieLabel(item, value, total) {
  var share = total ? Math.round(value / total * 100) : 0;
  var stated_label = SEGMENT_BASIS === 'profit' ? item && item.profit_mix || '' : item && item.mix || '';
  var m = String(stated_label).match(/(\d{1,3}(?:\.\d)?)\s*%/);
  if (m) {
    var stated = Number(m[1]);
...
function nbPieSVG(items, cx, cy, r) {
  var vals = (items || []).map(x => segmentShare(x)),
    total = vals.reduce((a, b) => a + b, 0) || 1;
  var cols = ['#98bf65', '#9fc3e7', '#ebc85d', '#baa2d2'];
  var a = 0;
  var out = [];
  vals.forEach((v, i) => {
    var a1 = a + v / total * 360;
    out.push(`<path class="nbv-pie-slice" d="${pieArc(cx, cy, r, a, a1)}" fill="${cols[i % cols.length]}" stroke="#3f3b34" stroke-width="1.05" filter="url(#nbRough)"/>`);
    var mid = (a + a1) / 2,
      pt = piePoint(cx, cy, r * .58, mid);
    out.push(`<text x="${pt[0].toFixed(1)}" y="${pt[1].toFixed(1)}" class="nbv-pie-label" text-anchor="middle">${esc(pieLabel(items[i], v, total))}</text>`);
    a = a1;
  });
  return out.join('');
}
function nbIcon(name, size = 24) {
  return `<span class="nbv-icon" style="width:${size}px;height:${size}px">${ICONS[name] || ICONS.product}</span>`;
}
var NUM_RE = /(\$?~?\d[\d,.]*(?:[-–]\d[\d,.]*)?%?x?\+?)/g;
...
function nbEarningsBridge(d) {
  var vis = (d.visuals || []).find(v => v.type === 'flow');
  var items = (vis?.items || []).slice(0, 4);
...
function notebookHTML(d, m) {
  ...
  var segCountCls = segs.length >= 4 ? ' nbv-seg-legend-4' : '';
  var segLegend = segs.map((x, i) => `<div class="nbv-seg"><span class="swatch s${i}"></span><div class="nbv-segcopy"><b>${esc(x.short_name || x.name)}</b><strong>${esc(x.mix)}</strong><small>${esc(x.description)}</small></div></div>`).join('');
  var thesis = `${nbSectionTitle('1', 'INVESTMENT THESIS')}<p class="lead">${esc(d.thesis_summary)}</p><div class="nbv-question">${esc(d.core_question)}</div><ul class="nbv-checks">${(d.thesis_bullets || []).map(x => `<li>${esc(x)}</li>`).join('')}</ul>${nbThesisArtwork(d)}`;
  var overview = `${nbSectionTitle('2', 'COMPANY OVERVIEW')}<p class="lead">${esc(d.overview_summary)}</p><div class="nbv-minihead">KEY SEGMENTS <span>${esc(segmentBasis(d))}</span></div><div class="nbv-seg-legend${segCountCls}">${segLegend}</div><div class="nbv-profit-note">${esc(d.other_profit_pool)}</div>`;
  var business = `${nbSectionTitle('3', 'BUSINESS MODEL', '— MULTIPLE PROFIT POOLS')}<div class="nbv-pools">${(d.business_model || []).slice(0, 4).map((x, i) => `${i ? '<i class="nbv-plus">+</i>' : ''}<div class="nbv-pool">${nbIcon(poolIconName(x.name), 28)}<b>${esc(x.name)}</b><span>${esc(x.description)}</span></div>`).join('')}</div><div class="nbv-life">Captures value across the customer life cycle. <b>→</b></div>`;
  var opps = `${nbSectionTitle('4', 'KEY OPPORTUNITIES')}<div class="nbv-opps">${(d.opportunities || []).slice(0, 5).map(x => `<div class="nbv-opp">${nbIcon(x.icon, 31)}<div><b>${esc(x.title)}</b><span>${esc(x.detail)}</span></div></div>`).join('')}</div>`;
  var financial = `${nbSectionTitle('5', 'FINANCIAL SNAPSHOT', '(FY / Latest)')}<div class="nbv-fin-top"><ul>${(d.financial_bullets || []).slice(0, 6).map(x => `<li>${nbRich(x)}</li>`).join('')}</ul><div class="nbv-target"><h3>${earningsLabels(d.earnings_history).targetsTitleCaps}</h3>${(d.targets || []).slice(0, 4).map(x => `<div><span>${esc(x.label)}</span><b>${esc(x.value)}</b></div>`).join('')}</div></div><div class="nbv-chart-title">${earningsLabels(d.earnings_history).chartTitle} <span>${esc(d.earnings_history?.metric || '')}</span></div><div class="nbv-fin-lower"><div class="nbv-chart">${earningsChartSVG(d.earnings_history, 'nbv-chart-svg')}<div class="nbv-cycle-note">${esc(d.earnings_history?.cycle_note || '')}</div></div><div class="nbv-val"><h3>VALUATION <span>(Today)</span></h3>${(d.valuation_metrics || []).slice(0, 4).map(x => `<div class="nbv-val-row"><span>${esc(x.label)}</span><b>${esc(x.value)}</b></div>`).join('')}<strong>${esc(d.valuation_callout)}</strong></div></div>`;
  var signs = `${nbSectionTitle('6', 'KEY SIGNPOSTS', '(WHAT TO WATCH)')}<table class="nbv-table sign"><thead><tr><th>SIGNPOST</th><th>CURRENT</th><th>TARGET / TRIGGER</th><th>WHY IT MATTERS</th></tr></thead><tbody>${(d.signposts || []).slice(0, 6).map(x => `<tr><td><b>${esc(x.signpost)}</b></td><td>${nbRich(x.current)}</td><td>${nbRich(x.target)}</td><td>${esc(x.why)}</td></tr>`).join('')}</tbody></table>`;
  var threats = `${nbSectionTitle('7', 'THESIS THREATS', '(WHAT COULD BREAK IT)')}<table class="nbv-table threat"><tbody>${(d.threats || []).slice(0, 4).map(x => `<tr><td class="ico">${nbIcon(x.icon, 25)}</td><td><b>${esc(x.threat)}</b></td><td>${esc(x.watch_for)}</td></tr>`).join('')}</tbody></table>`;
  var final = `${nbSectionTitle('★', 'FINAL TAKEAWAY')}<p class="nbv-finalcopy">${esc(d.final_takeaway)}</p><div class="nbv-cases"><div class="case bull"><h3>↗ BULL CASE</h3><ul>${listHTML(posterList((d.bull_case || []).slice(0, 5), 34))}</ul></div><div class="vs">VS.</div><div class="case bear"><h3>↓ BEAR CASE</h3><ul>${listHTML(posterList((d.bear_case || []).slice(0, 5), 34))}</ul></div></div>`;
...
function tpPieSVG(items) {
  var vals = (items || []).map(x => segmentShare(x)),
    total = vals.reduce((a, b) => a + b, 0) || 1;
  var cols = ['#89b45b', '#92bce0', '#e6c255', '#b69bd0'];
  var a = 0;
  return `<svg class="tp-pie-svg" viewBox="0 0 300 300" preserveAspectRatio="xMidYMid meet">${vals.map((v, i) => {
    var a1 = a + v / total * 360,
      mid = (a + a1) / 2,
      pp = piePoint(150, 150, 68, mid);
    var out = `<path d="${pieArc(150, 150, 104, a, a1)}" fill="${cols[i % cols.length]}" stroke="#4a463f" stroke-width="1.15"/><text x="${pp[0]}" y="${pp[1]}" text-anchor="middle" dominant-baseline="middle">${esc(items[i].mix || '')}</text>`;
    a = a1;
    return out;
  }).join('')}</svg>`;
}
function twopagerNotebookHTML(d, m) {
  setSegmentBasis(d.segments);
  var segs = (d.segments || []).slice(0, 4);
  var p1 = `<article class="tp-page tp-notebook-page tp-p1">${tpHeader(d, 1, 'Franchise, investment case and upside drivers')}<main class="t16-p1-grid">
  ...
  var p2 = `<article class="tp-page tp-notebook-page tp-p2">${tpRunningHeader(d, 2, 'Earnings power, signposts and thesis-break conditions')}<main class="t16-p2-grid">
...
function twopagerInstitutionalHTML(d, m) {
  return twopagerNotebookHTML(d, m).replaceAll('tp-notebook-page', 'tp-institutional-page');
}
function renderTwopager() {
  if (!CURRENT || !$('twopagerView')) return;
  var d = CURRENT.onepager,
    m = CURRENT.master,
    t = $('twopagerTemplate').value;
...
function institutionalHTML(d, m) {
  var f = m.financial_snapshot || {},
    t = m.investment_thesis || {};
  var metrics = [['Price', m.at_glance?.share_price], ['Mkt Cap', m.at_glance?.market_cap], ['Fwd P/E', f.forward_pe], ['Revenue', f.revenue], ['Margin', f.operating_margin], ['EPS', f.eps]];
...
function dashboardHTML(d, m) {
  var f = m.financial_snapshot || {};
  var seg = d.segments || [];
  var kpis = [['PRICE', m.at_glance?.share_price], ['MARKET CAP', m.at_glance?.market_cap], ['FWD P/E', f.forward_pe], ['REVENUE', f.revenue], ['OP MARGIN', f.operating_margin], ['EPS', f.eps]];
...
function strategyHTML(d, m) {
  var t = m.investment_thesis || {};
  var vals = m.valuation_scenarios || [];
  var cats = m.catalysts || [];
  return `<article class="op-canvas strategy-canvas">
    ...
    <section class="st-bridge"><div class="st-label">EARNINGS BRIDGE</div>${flowHTML((d.visuals || []).find(x => x.type === 'flow')?.items || [{
    label: 'Cycle',
    detail: 'Demand'
  }, {
    label: 'Mix',
    ...
    label: 'EPS',
    detail: 'Power'
  }], 'strategy-flow')}<div class="st-targets">${(d.targets || []).map(x => `<div><span>${esc(x.label)}</span><b>${esc(x.value)}</b></div>`).join('')}</div></section>
...
function renderOnepager() {
  ...
  var d = CURRENT.onepager,
    m = CURRENT.master,
    t = $('onepagerTemplate').value;
  var renderers = {
    notebook: notebookHTML,
    institutional: institutionalHTML,
    dashboard: dashboardHTML,
    strategy: strategyHTML,
...
function fitNotebookLayout() {
  var canvas = $('onepagerView').querySelector('.notebook-svg-canvas');
  if (!canvas) return;
  canvas.dataset.layoutNeed = '1536';
  canvas.dataset.layoutAvailable = '1536';
}
function checkOnepagerFit() {
  if ($('onepagerTemplate').value === 'notebook') fitNotebookLayout();
  var canvas = $('onepagerView').querySelector('.op-canvas');
  if (!canvas || !CURRENT || OUTPUT !== 'onepager') return;
  var sections = [...canvas.querySelectorAll('.nb-box')];
  var sectionOverflow = sections.some(el => el.scrollHeight > el.clientHeight + 2 || el.scrollWidth > el.clientWidth + 2);
  var overflow = canvas.scrollHeight > canvas.clientHeight + 2 || canvas.scrollWidth > canvas.clientWidth + 2 || sectionOverflow;
  canvas.classList.toggle('overflow-warning', overflow);
  var apiWarnings = CURRENT.meta?.onepager_violations?.length || 0;
...
function showSources() {
  ...
  var src = CURRENT.master?.sources || [];
  ...
  var rows = src.length ? src.map((s, i) => `<li><a href="${safeUrl(s.url)}" target="_blank">${esc(s.title || s.url)}</a>${s.date ? ` <small>${esc(s.date)}</small>` : ''}</li>`).join('') : '<p>No source list is stored in this calibration dataset.</p>';
...
function rebalanceGridRows(grid, children, weights, safety = 10) {
  ...
  var pt = parseFloat(cs.paddingTop) || 0,
    pb = parseFloat(cs.paddingBottom) || 0;
  var available = grid.clientHeight - pt - pb - gap * (children.length - 1);
  var desired = children.map(el => Math.ceil(measuredContentNeed(el) + safety));
  var sum = desired.reduce((a, b) => a + b, 0);
  if (sum > available + 2) {
  ...
  var extra = Math.max(0, available - sum);
  var ww = weights && weights.length === children.length ? weights : children.map(() => 1);
  var wsum = ww.reduce((a, b) => a + b, 0) || 1;
  var rows = desired.map((d, i) => d + extra * ww[i] / wsum);
...
function rebalanceLongform() {
  ...
  var rp1 = document.querySelector('.v21-report-p1');
  ...
  var tp1 = document.querySelector('.t16-p1-grid');
  ...
  var tp2 = document.querySelector('.t16-p2-grid');
...
function strictClipFailures() {
  var failures = [];
  var visible = [...document.querySelectorAll('.strict-fit')].filter(el => el.offsetParent !== null);
  visible.forEach(el => {
    var er = el.getBoundingClientRect();
    var tol = 6;
    ...
    var candidates = [...el.querySelectorAll('tbody tr:last-child, .report-sensitivity small, .report-sensitivity tbody tr:last-child, article:last-child, .report-decision-lens, .v21-bottom-line')];
...
function collectLayoutQA() {
  var mode = OUTPUT;
  if (mode === 'onepager') {
    var canvas = document.querySelector('.onepager-view:not(.hidden) .op-canvas');
    if (!canvas) return {
      failures: [],
      warnings: []
    };
    var _failures = canvas.classList.contains('overflow-warning') ? ['One-pager: layout overflow warning'] : [];
    return {
      failures: _failures,
      warnings: []
    };
  }
  var sel = mode === 'report' ? '.report-page' : '.tp-page';
  var pages = [...document.querySelectorAll(sel)].filter(p => p.offsetParent !== null);
  var failures = [...strictClipFailures()],
    warnings = [];
  pages.forEach((page, idx) => {
    var pr = page.getBoundingClientRect();
    var footer = page.querySelector('.report-footer, footer');
    var fr = footer ? footer.getBoundingClientRect() : null;
    // Hard failures only: page-boundary escape or real footer collision.
    var protectedNodes = [...page.querySelectorAll('section, table, .report-pie-wrap, .tp-pie-wrap, .report-earnings-chart, .tp-chart-row')];
    protectedNodes.forEach(n => {
      ...
      var clips = /hidden|clip/.test(cs.overflow + cs.overflowY + cs.overflowX);
      var isStrict = n.classList && n.classList.contains('strict-fit');
      var realClip = isStrict ? directContentOverflow(n, 6) : n.scrollHeight > n.clientHeight + 6 || n.scrollWidth > n.clientWidth + 6;
      if (clips && realClip) {
    ...
    var body = page.querySelector('.r16-p1-grid,.r16-p2-grid,.r16-p3-grid,.t16-p1-grid,.t16-p2-grid');
    if (body) {
      var br = body.getBoundingClientRect();
      var content = [...body.children].filter(x => x.getBoundingClientRect().height > 0);
      if (content.length) {
        var maxBottom = Math.max(...content.map(x => x.getBoundingClientRect().bottom));
        var used = Math.max(0, Math.min(1, (maxBottom - br.top) / br.height));
      ...
      [...body.querySelectorAll('section')].forEach(sec => {
        ...
        var kids = [...sec.children].filter(x => x.getBoundingClientRect().height > 0);
        if (sr.height > 180 && kids.length) {
          var last = Math.max(...kids.map(x => x.getBoundingClientRect().bottom));
          var blank = sr.bottom - last;
          if (blank > 150) warnings.push(`Page ${idx + 1}: ${Math.round(blank)}px unused in ${sec.className || 'section'}`);
        }
      });
      var selector = mode === 'report' ? '.report-section p,.report-section li,.report-table td,.report-evidence span,.report-decision-matrix td' : '.tp-page p,.tp-page li,.tp-signposts td,.tp-threats p,.tp-cases li';
      var sizes = [...page.querySelectorAll(selector)].filter(x => x.textContent.trim()).map(x => parseFloat(getComputedStyle(x).fontSize)).filter(Number.isFinite).sort((a, b) => a - b);
      if (sizes.length) {
        var med = sizes[Math.floor(sizes.length / 2)],
          floor = mode === 'report' ? 14 : 14.2;
...
function updateLayoutQA() {
  if (!CURRENT) return;
  var qa = collectLayoutQA();
  var suffix = qa.failures.length ? ` · QA: ${qa.failures.length} issue(s)` : qa.warnings.length ? ` · QA: ${qa.warnings.length} layout note(s)` : ' · QA PASS';
  var base = OUTPUT === 'report' ? `${CURRENT.master.company} · ${$('reportTemplate').selectedOptions[0].textContent}` : OUTPUT === 'twopager' ? `${CURRENT.master.company} · ${$('twopagerTemplate').selectedOptions[0].textContent}` : `${CURRENT.master.company} · ${$('onepagerTemplate').selectedOptions[0].textContent}`;
...
function exportJSON() {
  ...
  a.download = `${CURRENT.master.ticker}_research_v23.json`;
  a.click();
  URL.revokeObjectURL(a.href);
}
async function printCurrent() {
  if (!CURRENT) return alert('Run an analysis first.');
  var cls = OUTPUT === 'report' ? 'print-report' : OUTPUT === 'onepager' ? 'print-onepager' : 'print-twopager';
  document.body.classList.add(cls);
  await new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r)));
  rebalanceLongform();
  await new Promise(r => requestAnimationFrame(r));
  var hard = strictClipFailures();
  var qa = collectLayoutQA();
  if (hard.length) {
...
export function setOutput_(kind) {
  OUTPUT = kind;
}
export { notebookHTML, institutionalHTML, dashboardHTML, strategyHTML, editorialHTML, twopagerNotebookHTML, twopagerInstitutionalHTML, renderReport, earningsChartSVG, esc, listHTML,
...
export function memoHTML(master, onepager) {
  ...
  var prior = document.getElementById('reportView');
