...
def retrieve(cur,ticker,case,recent,as_of,focus=''):
    ...
    cur.execute('''SELECT d.id,d.revision,d.body,d.created_at,
        ...
        (SELECT count(*) FROM unnest(%s::text[]) term WHERE to_tsvector('english',concat_ws(' ',d.body->>'decision',
            d.body->>'rationale',d.body->>'revisitWhen',d.body->>'issue')) @@ plainto_tsquery('english',term)) AS relevance
    ...
    return selected,{'method':'stemmed_question_and_case_terms' if focus else 'stemmed_case_terms','terms':terms,'termsLimited':limited,
        'selectedIds':[str(r['id']) for r in selected],
        'additionalMatchesOmitted':len(rows)>12 and rows[12]['relevance']>0,
        'scope':'Up to 12 older matching decisions in addition to the latest 20. English stemming can match word variants but can miss conceptual relationships. No original documents or meeting answers retrieved.'}
