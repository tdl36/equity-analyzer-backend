...
export function EvidenceWorkspace({api,analyses,onCompany}) {
  ...
  return <section className="evidence-workspace">
    ...
    {!loading&&!error&&(data?.savedThesis||review)&&<details className="workspace-panel"><summary>Discuss this research with an analyst</summary><ResearchChat key={`${ticker}:${refresh}`} api={api} context={{ticker,type:review?'review':'thesis',content:JSON.stringify(review?.state||data.savedThesis)}}/></details>}
