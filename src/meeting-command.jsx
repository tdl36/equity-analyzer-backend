...
export function MeetingCommand({api,onNavigate,onSection}){
  ...
  return <section className="workspace-panel meeting-command">
    ...
    <div className="meeting-pack-list">{jobs.map(j=><article key={j.id}><div><strong>{j.ticker}</strong><span>{j.options?.meetingPrep?.meetingDate}</span></div><p>{j.prep_status==='done'?'Pack ready':j.prep_status==='running'?`Preparing questions · ${j.prep_step||'starting'}${j.completed?` (${j.completed}/${j.total})`:''}`:j.prep_status==='queued'?'Sources verified · waiting for preparation':j.prep_status==='failed'?'Preparation needs attention':j.status==='failed'?'Collection request failed':j.status==='queued'?'Waiting for Mac acknowledgment':'Collecting sources / preparing analyst brief'}</p>{(j.prep_error||j.meeting_issue||j.error)&&<p className="workspace-error">{j.prep_error||j.meeting_issue||j.error}</p>}{j.meeting_id&&<button onClick={()=>{window.dispatchEvent(new CustomEvent('charlie-open-meeting',{detail:{id:Number(j.meeting_id)}}));onNavigate('meetingprep');}}>Open {j.ticker} meeting →</button>}{j.prep_status==='failed'&&<button disabled={busy} onClick={()=>retry(j.job_id)}>Retry meeting pack</button>}<button onClick={()=>onSection('collection')}>Collection details →</button></article>)}</div>
  </section>;
}
