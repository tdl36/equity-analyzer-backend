...
const SECTIONS=[['brief','Executive Brief','brief'],['takeaways','Key Takeaways','summary'],['meeting','Meeting Summary','meeting_summary'],['questions','Follow-up Questions','questions'],['assessment','Overall Assessment','assessment']];
...
export function SummaryLab({api,getKey,getGeminiKey,renderHtml,pickFromICloud}){
 ...
 const [intake,setIntake]=useState('saved'),[audioFile,setAudioFile]=useState(null),[youtubeUrl,setYoutubeUrl]=useState(''),[youtubeTicker,setYoutubeTicker]=useState(''),[ingest,setIngest]=useState(null);
 const [expanded,setExpanded]=useState({brief:true,takeaways:true,meeting:false,questions:false,assessment:false});
 ...
 async function startLab(summaryId,labTitle){
  const d=await req('',{summaryId:summaryId||undefined,source:summaryId?undefined:source,title:(labTitle||title).trim()||'Untitled experiment',focus,apiKey:getKey()});
  setId(d.id);await refreshLists();return d;
 }
 async function start(){setBusy(true);setError('');setNotice('');try{await startLab(sid||undefined,title);}catch(e){setError(e.message);}finally{setBusy(false);}}
 ...
 async function monitorJob(jobId,label,jobTitle=title,jobFocus=focus){
  ...
  try{
   for(let i=0;i<1080;i++){
    ...
    localStorage.setItem(PENDING_KEY,JSON.stringify({jobId,label,title:jobTitle,focus:jobFocus}));
    if(['complete','done'].includes(phase)){
     ...
     await startLab(data.summaryId,jobTitle||label);setIngest(null);setBusy(false);return;
    }
 ...
 async function processAudio(){
  ...
  try{
   ...
   const label=audioFile.name,jobTitle=title||label;localStorage.setItem(PENDING_KEY,JSON.stringify({jobId:data.jobId,label,title:jobTitle,focus}));setBusy(false);await monitorJob(data.jobId,label,jobTitle,focus);
  }catch(e){setError(e.message);setBusy(false);}
 }
 async function processYoutube(){
  setBusy(true);setError('');setNotice('');
  try{
   const response=await fetch(`${api}/api/youtube-summarize`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({url:youtubeUrl.trim(),ticker:youtubeTicker.trim().toUpperCase(),apiKey:getKey()}),signal:AbortSignal.timeout(30000)});const data=await response.json();if(!response.ok)throw Error(data.error||'YouTube processing could not start.');
   const label=data.title||'YouTube video',jobTitle=title||label;if(!title.trim())setTitle(label);localStorage.setItem(PENDING_KEY,JSON.stringify({jobId:data.jobId,label,title:jobTitle,focus}));setBusy(false);await monitorJob(data.jobId,label,jobTitle,focus);
  }catch(e){setError(e.message);setBusy(false);}
 }
 ...
 function openEmail(){setEdits({...state.sections});setSharing(true);setCompare(false);setExpanded(Object.fromEntries(SECTIONS.map(([key])=>[key,true])));setNotice('Review and edit each section before sending. Edits affect this email only.');}
 ...
 <div className="eyebrow">Charlie / Research experiments</div><h1>Summary Lab</h1><p>Read thoroughly. Preserve what was said. Separate what it means.</p><p className="muted">An independent trial with five familiar sections. Original summaries and automatic workflows remain unchanged.</p>
 ...
 <section className="panel" style={{marginTop:20}}><h2>Experiments</h2>{!runs.length&&<p className="muted">Your experiments will appear here.</p>}{runs.map(r=><button className="experiment" aria-pressed={id===r.id} key={r.id} onClick={()=>{setId(r.id);setNotice('');}}>{r.title}<div className="muted">{r.status==='complete'?'Ready to review':r.status} · {new Date(r.created_at).toLocaleDateString()}</div></button>)}</section></aside>
 ...
 <div className="controls"><button onClick={()=>setExpanded(Object.fromEntries(SECTIONS.map(([key])=>[key,true])))}>Expand all</button><button onClick={()=>setExpanded(Object.fromEntries(SECTIONS.map(([key])=>[key,false])))}>Collapse all</button></div>{notice&&<p role="status">{notice}</p>}
 {sharing&&<section className="status"><strong>Email preview · all five sections</strong><p>Review any section below before sending. Edits affect this email only; audit notes and the full transcript are excluded.</p><div className="controls"><button className="primary" disabled={sending} onClick={sendEmail}>{sending?'Sending…':'Send all sections to myself'}</button><button disabled={sending} onClick={()=>{setSharing(false);setNotice('');}}>Close email preview</button></div></section>}
 {SECTIONS.map(([key,label,baseline])=>{const value=(sharing?edits[key]:state.sections?.[key])||'';return <details key={key} className="lab-section" open={!!expanded[key]} onToggle={e=>setExpanded(current=>({...current,[key]:e.currentTarget.open}))}><summary><span><span className="chevron" aria-hidden="true">›</span> <strong>{label}</strong></span><span className="muted">{value?'Ready':'Waiting'}</span></summary><div className="section-body"><div className="controls"><button disabled={!value} onClick={()=>copy(false,key)}>Copy section</button></div>{row.status!=='complete'&&value&&<p className="muted">Draft in progress. Source checks may still revise this section.</p>}{sharing?<><label htmlFor={`lab-edit-${key}`}>Edit for this email</label><textarea className="email-editor" id={`lab-edit-${key}`} value={value} onChange={e=>setEdits({...edits,[key]:e.target.value})}/><div className="reader" dangerouslySetInnerHTML={{__html:documentHtml(value||'This section is not available.',renderHtml)}}/></>:<div className={compare?'pair':''}>{compare&&<article><h3>Original · frozen at experiment start</h3><div className="original" dangerouslySetInnerHTML={{__html:renderHtml(row.baseline?.[baseline]||'<p>No original section saved.</p>')}}/></article>}<article>{compare&&<h3>Improved</h3>}<div className="reader" dangerouslySetInnerHTML={{__html:documentHtml(value||'This section will appear after source review and generation.',renderHtml)}}/></article></div>}</div></details>;})}
