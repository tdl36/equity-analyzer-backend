  ...
  return <section className="earnings-workspace">
    ...
    <div className="earnings-layout"><aside className="workspace-panel"><label className="earnings-search">Find an event<input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Ticker, event or analyst"/></label>
      ...
      {event&&<article className="workspace-panel earnings-detail" key={event.id}><p className="workspace-eyebrow">{event.ticker} / {labels[event.state]}</p><h2>{event.input.topic}</h2>
        ...
        <CatalystComparison value={event.output?.catalystComparison} jobId={event.output?.catalystJobId} ticker={event.ticker} api={api} renderHtml={renderHtml} />
        ...
        {event.output?.evidenceSnapshot?.version===1&&<details open><summary>Recorded synthesis inputs · {event.output.evidenceSnapshot.sources?.length||0} documents</summary><p className="desk-explainer">Captured when this draft was generated. This records source delivery, not claim verification.</p><ul>{(event.output.evidenceSnapshot.sources||[]).map((s,i)=><li key={i}><strong>{s.filename}</strong> · {s.inputMode==='native_pdf'?'Native PDF':s.inputMode==='extracted_text'?'PDF text extraction':'Text'}{s.pages?` · ${s.pages} pages`:''}{s.characters!=null?` · ${s.characters.toLocaleString()} characters`:''}{s.textExtraction?.ocrPages?.length?` · OCR pages: ${s.textExtraction.ocrPages.join(', ')}`:''}{s.textExtraction?.limitations?.map((issue,n)=><p key={n} className="desk-explainer">{issue}</p>)}<br/><small>SHA-256: {s.sha256}</small></li>)}</ul></details>}
        {event.output?.coordination&&<details><summary>Coordinated research review · {event.output.coordination.issues?.length||0} challenge findings</summary><p>{event.output.coordination.scope}</p><p>{event.output.coordination.roles?.map(r=>`${r.role}: ${r.status.replaceAll('_',' ')}`).join(' · ')}</p>{event.output.coordination.issues?.map(issue=><article className="amendment-card" key={issue.id}><h4>Challenge {issue.id}</h4><blockquote>{issue.draftQuote}</blockquote><p>{issue.issue}</p><p>{issue.recommendation}</p>{event.output.coordination.decisions?.filter(d=>d.id===issue.id).map(d=><p key={d.id}><strong>{d.action}</strong> · {d.reason}</p>)}</article>)}<details><summary>Lead draft before challenge review</summary><pre style={{whiteSpace:'pre-wrap',maxHeight:440,overflow:'auto'}}>{event.output.coordination.leadDraft}</pre></details></details>}
        {event.output?.processingRecovery&&<p className="desk-explainer">Processing record: {event.output.processingRecovery.totalBatches} source batches · {event.output.processingRecovery.resumedBatches} reused from a matching recovery checkpoint.</p>}
