...
## T30 Guided meeting and conference packs

Command Charlie now opens a guided Meeting & conference prep form: choose 1–10 covered companies with checkboxes, a meeting date, 30/60/90-day source window and optional focus areas/notes. The backend generates the instruction; no prompt syntax is required. Existing research favorites remain under Investigate & update research. Meeting Prep also links to the guided form. Each batch has immutable, deterministic per-company request IDs; retries after uncertain submissions reuse those IDs. All coverage is validated before any command is inserted.

Each command uses the existing managed collection and recap route into its unique CATALYSTS folder, then a Mac heartbeat dispatches a meeting pack only when the exact recap has a verified source register. Up to 20 originals / 60 MB are copied into a dedicated meeting session in the same transaction as its durable job. No missing source is silently dropped. Sources are rehashed at preparation and execution; native PDFs retain charts/visuals for the existing document-analysis path. Non-PDF text has explicit size bounds. Saved thesis context (bounded and disclosed) and previous questions feed synthesis. Questions require rationale, priority, follow-up and exact registered source filenames; this checks citation identity, not factual entailment.

One PostgreSQL-owned managed meeting worker runs across server processes; within a pack up to four document analyses can run concurrently. Stage checkpoints survive failures; abandoned execution resumes at most twice, while failed packs offer an explicit retry. A saved meeting receipt prevents duplicate question sets after interrupted final delivery. Generated managed questions are recorded as planned, never as already asked. Existing meeting outputs are not overwritten.

Boundaries: browser pickup remains scheduled; no instant collection, cache-first preview, cross-company simultaneous pack generation or arbitrary IR/presentation adapter is claimed. Requested presentation coverage may be missing and must be disclosed. The route currently waits for the analyst recap before preparing questions. A shared meeting date applies to the selected batch; separate dates require separate batches. Real paid end-to-end packs and full-coverage browser collection remain to be validated. No real assignment is launched solely for testing.

T31 verification hardening: managed meeting progress and final saves are fenced by a unique worker token as well as the PostgreSQL execution lock. A disconnected/stale worker cannot publish after a replacement claims the job. Generated output with an invalid source register discards only the question-stage checkpoint so retry regenerates questions while retaining analyses and synthesis. Transient form-load errors clear after a successful refresh. No live paid meeting assignment was created during validation.
