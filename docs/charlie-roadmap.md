...
Follow-up inspection fixed managed question generation dropping the custom
assignment context after source analysis, and retained cache-hit counts in the
final receipt. Targeted orchestration tests verify both behaviors without paid
model calls. The earlier saved ABT pack is not silently regenerated by this fix.

## T40 — Saved-source meeting briefs and My work

New completed-pack reuse verifies every source hash, clones originals into a separate
meeting and queues managed preparation directly, without AlphaSense collection or
a new recap prerequisite. Existing source policies and source cutoff are inherited
and disclosed; eligible analysis cache entries are reused, with new synthesis and
questions. Request identity prevents duplicate submissions; prior packs remain
unchanged. This is source reuse, not proof that the sources remain current.

My work is the default Research desk section. It combines recent meeting assignments,
research commands and analyst inbox/failed items with status filters, ticker search,
assignment inspection, direct question links and inline completed recap reading.
Collection and generation status remain separate. Broader historical outputs remain
in Research history; this is not an exhaustive inventory of every legacy workflow.

Remaining: broaden source/quality benchmark annotations and measured latency;
complete real conference acceptance; direct meeting-question agent revision with
version comparisons; per-company conference dates; improved primary IR coverage;
