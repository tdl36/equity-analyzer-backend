...
def command(data):
    ...
    if action=='save':
        if type(payload.get('hours'))!=int or payload['hours'] not in (0,1,4,12,24,168):raise ValueError('Invalid refresh frequency.')
        ...
        if not isinstance(kinds,list) or not kinds or any(k not in ('transcript','broker-report') for k in kinds):raise ValueError('Choose source types.')
