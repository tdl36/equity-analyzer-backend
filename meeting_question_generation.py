...
def generate(api_key,ticker,company_name,sector,synthesis,unresolved,source_names,client=None,evidence=None,meeting_profile=None,revision_context=None,on_progress=None,company_context=None):
    ...
    prompt=(profile_instruction(meeting_profile)+'Prepare prioritized questions for a professional equity investor meeting '+ticker+' ('+company_name+'). '
            ...
            'Keep broker names in private context/source metadata, but preserve the evidence type and uncertainty in the spoken question (for example, preliminary single-center feedback). Do not invent page references, launch dates, historical answers or head-to-head clinical comparisons. '
