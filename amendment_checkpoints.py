...
class Checkpoints:
    def __init__(self,get_db,job_id):self.get_db=get_db;self.job_id=job_id
    def load(self,key,stage):
        with self.get_db() as (_,cur):
            cur.execute("SELECT status,result FROM mp_jobs WHERE id=%s AND stage='evidence_amendment'",(self.job_id,))
            row=cur.fetchone()
        if not row or row['status']!='running':raise ValueError('Proposal was stopped; no additional model work permitted')
    ...
    def save(self,key,stage,value):
        ...
        with self.get_db(commit=True) as (_,cur):
            cur.execute("SELECT status,result FROM mp_jobs WHERE id=%s AND stage='evidence_amendment' FOR UPDATE",(self.job_id,))
            row=cur.fetchone()
            if not row or row['status']!='running':raise ValueError('Proposal was stopped; late stage output was not saved')
            result=row.get('result') or {}
            if isinstance(result,str):result=json.loads(result)
            saved=result.get('checkpoint',{})
