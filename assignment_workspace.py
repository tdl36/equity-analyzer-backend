    ...
    def detail(cid):
        ...
        with get_db() as (_,cur):
            ...
            if prep:
                ...
                checkpoint=obj(prep['result']) or {};prep={k:prep[k] for k in ('id','status','error','created_at','updated_at')}|{'stage':checkpoint.get('stage'),'completed':checkpoint.get('completed'),'total':checkpoint.get('total'),'cachedSources':sum(bool(x and x.get('_cacheHit')) for x in checkpoint.get('analyses',[]))}
            return jsonify(command={k:command[k] for k in ('id','ticker','status','error','created_at','updated_at')},assignment=payload,collection=collection,selection=obj(selection['value']) if selection else None,preparation=prep,meetingId=meeting,documents=documents,versions=versions)
