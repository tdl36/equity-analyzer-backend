...
p=argparse.ArgumentParser();p.add_argument('--command',required=True);p.add_argument('--file');args=p.parse_args()
url=agent.CHARLIE_API+'/api/research/commands/'+args.command+'/source-shortlist'
if args.file:
    value=json.loads(Path(args.file).read_text())
    r=requests.post(url,headers=agent._agent_headers(),json={'candidates':value},timeout=30)
